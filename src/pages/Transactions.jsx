    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-4 items-stretch">
        ...
        <div>
         ...
         <div className="flex gap-8 border-b border-gray-200 mb-4 ml-5">
            {tabs.map(tab => (
              <button
                key={tab}
          ...
          {/* Tabel dalam kotak putih */}
          <div className="bg-white rounded-2xl p-6 ml-5">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-[#718EBF] text-mb">
                  ...
                  <th className="pb-4 font-medium pr-0 text-center">Receipt</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map((t, i) => (
                  <tr key={i} className="border-t border-gray-100 hover:bg-gray-50 transition-colors">
                    <td className="py-4 flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center ${t.income ? 'bg-green-50' : 'bg-red-50'}`}>
                        <span className={`text-lg ${t.income ? 'text-green-500' : 'text-red-400'}`}>
                          {t.income ? '↓' : '↑'}
                        </span>
                      </div>
                      <span className="text-[#343C6A] font-medium">{t.desc}</span>
                    </td>
                    <td className="py-4 text-[#]">{t.id}</td>
                    <td className="py-4 text-[#]">{t.type}</td>
                    <td className="py-4 text-[#]">{t.card}</td>
                    <td className="py-4 text-[#]">{t.date}</td>
          ...
          {/* Pagination*/}
          <div className="flex justify-end items-center mt-6 ml-5">
